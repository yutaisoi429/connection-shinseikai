...
export function createApi({ store = new JsonStore(process.env.DATA_FILE || 'data/database.json'), allowedOrigin = process.env.CORS_ORIGIN || '*' } = {}) {
  return createServer(async (req,res) => {
    ...
    try {
      ...
      if (params && req.method==='POST') { const input=await body(req); required(input.title,'title'); required(input.assigneeIds,'assigneeIds'); const task={id:randomUUID(),projectId:params.projectId,title:input.title,description:input.description||'',status:'open',dueAt:input.dueAt||null,assigneeIds:[...new Set(input.assigneeIds)],lastActivityAt:new Date().toISOString(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}; await store.update(db=>{db.tasks.push(task); for(const userId of task.assigneeIds) db.notifications.push({id:randomUUID(),userId,eventType:'task.assigned',resourceType:'task',resourceId:task.id,title:'タスクが割り当てられました',readAt:null,createdAt:new Date().toISOString()});}); return json(res,201,task); }
